...
namespace CipherBreakerApi.Store
{
	...
	{
		const string CipherBreakerTableName = "cb";
		...
		public void InsertWordFreq(string word, long frequency)
		{
			...
			command.CommandText = $"insert into {CipherBreakerTableName} values({word},{frequency})";
		...
		{
			var command = CreateCommand();
			command.CommandText = $"update {CipherBreakerTableName} set frequency={frequency} where word={word}";
			command.ExecuteNonQuery();
		}
		...
		{
			var command = CreateCommand();
			command.CommandText = $"select frequency from {CipherBreakerTableName} where word={word}";
		...
		{
			var command = CreateCommand();
			command.CommandText = $"select * from {CipherBreakerTableName}";
		...
		{
			var command = CreateCommand();
			command.CommandText = $"delete from {CipherBreakerTableName} where word={word}";
			command.ExecuteNonQuery();
		}
